...
export default function App() {
  ...
  const performers = useMemo(() => {
    ...
    episodes.forEach((episode) => episode.performers.forEach((performer) => {
      if (!appearances.has(performer)) appearances.set(performer, new Set());
      appearances.get(performer)!.add(`${episode.date ?? ""}\u0000${episode.project}`);
    }));
    return [...appearances.keys()].sort((a, b) =>
      appearances.get(b)!.size - appearances.get(a)!.size || a.localeCompare(b, "ja"),
    );
  }, []);
  ...
  const selectedEpisodes = useMemo(() => {
    const records = selectedCountry
      ? filteredEpisodes.filter((episode) => episode.countryCode === selectedCountry.countryCode)
      : selectedPerformer ? filteredEpisodes : [];
    const broadcasts = new Map<string, Broadcast>();
    records.forEach((episode) => {
      const key = `${episode.date ?? ""}\u0000${episode.project}`;
  ...
  }, [filteredEpisodes, selectedCountry, selectedPerformer]);
  ...
  return (
    <main>
      ...
      <aside className={`selection ${selectedCountry ? "is-visible" : ""}`} aria-live="polite">
        {selectedCountry ? <><p>選択した国</p><strong>{selectedCountry.countryName}</strong><span>{selectedCountry.visits > 0 ? `訪問件数 ${selectedCountry.visits}件` : selectedPerformer ? "該当する訪問記録なし" : "未訪問"}</span></> : <p>{selectedPerformer ? `${selectedPerformer}の訪問国と放送リストを表示しています。` : "地図から国を選ぶと、ここに表示されます。"}</p>}
      </aside>
      {(selectedCountry || selectedPerformer) && (
        <section className="broadcasts" aria-labelledby="broadcasts-title">
          <div className="broadcasts-heading">
            <div><p className="section-label">BROADCAST LIST</p><h2 id="broadcasts-title">{selectedCountry?.countryName ?? selectedPerformer}の放送リスト</h2></div>
